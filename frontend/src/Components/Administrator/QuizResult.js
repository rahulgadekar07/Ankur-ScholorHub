...
const QuizResult = () => {
  ...
  useEffect(() => {
    const fetchQuizResults = async () => {
      try {
        const response = await fetch("http://localhost:5000/quiz/getQuizResults"); // Adjust the URL as per your backend API endpoint
        if (!response.ok) {
        ...
        // Calculate total users, passed, and failed
  ...
  // Function to format timestamp to 'dd/mm/yyyy 00:00 am/pm' format
