...
const AdminApplicationForm = () => {
  ...
  useEffect(() => {
    ...
    const fetchIncomeDetails = async () => {
      try {
        const response = await fetch(
          `http://localhost:5000/scholarship/getAllIncomeDetails/${userId}`
        );
        const data = await response.json();
    ...
    const fetchAddressDetails = async () => {
      try {
        const response = await fetch(
          `http://localhost:5000/scholarship/getAllAddressDetails/${userId}`
        );
        const data = await response.json();
        if (data[0]) {
  ...
  const fetchUserData = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:5000/user/getUserData?userId=${userId}`, {
        method: "GET",
        headers: {
