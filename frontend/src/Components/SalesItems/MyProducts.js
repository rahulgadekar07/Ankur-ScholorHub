...
const MyProducts = () => {
  ...
  const fetchProducts = async () => {
    try {
      const response = await fetch(
        `http://localhost:5000/sales/productsbyid?userId=${userId}`,
        {
          method: "GET",
        }
      );
  ...
  try {
    ...
    const response = await fetch(`http://localhost:5000/sales/editproduct`, {
      method: "PUT",
      body: formData,
    });
          ...
          <div key={index} className="card mb-3 mx-2" style={{maxWidth:'30vw'}}>
            <div className="row g-0">
              <div className="col-md-4">
                <img
                  src={`http://localhost:5000/${product.image}`} // Assuming the product object has an 'image' property
                  alt="Product"
                  className="img-fluid"
                  style={{
